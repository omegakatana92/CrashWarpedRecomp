...
A modern fan-made recompilation project for **Crash Bandicoot: Warped**, originally released for the **Sony PlayStation**.
...
To play, you must provide your **own legally obtained copy** of *Crash Bandicoot: Warped*.
...
* `launcher\crashWarped.bat` â€” double-click to start.
* `launcher\crashWarped.ps1` â€” the actual PowerShell + WinForms GUI.

The launcher lets you:

1. Pick your legally obtained Crash Warped `.cue` file (Browse...).
...
`%APPDATA%\%CrashWarpedRecomp\launcher-config.json` and re-loaded the next time
the launcher starts.
...
```bash
git clone https://github.com/omegakatana92/CrashWarpedRecomp.git
cd CrashWarpedRecomp
```
...
**Crash Bandicoot: Warped**, its characters, names, artwork, music, trademarks, and other intellectual property remain the property of their respective copyright and trademark holders.
