...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\launcher\crashWarped.ps1
...
#     %APPDATA%\CrashWarpedRecomp\launcher-config.json (per-user). Falls back
...
[CmdletBinding()]
param(
    [string]$RecompExeName = 'Crash_Warped_Recompiled.exe'
)
...
function Get-ConfigDir {
    ...
    $dir = Join-Path $appData 'CrashWarpedRecomp'
...
$form.Text = 'Crash Warped Launcher'
...
$lblInfo.Text = "Select your legally obtained Crash Bandicoot - Warped disc image (.cue).
...
$btnBrowse.Add_Click({
    ...
    $dlg.Title = 'Select your legally obtained Crash Bandicoot - Warped disc image'
...
function Show-Error {
    ...
    [System.Windows.Forms.MessageBox]::Show($form, $Message, 'Crash Warped Launcher', 'OK', 'Warning') | Out-Null
}
